                ...
                with self._lock:
                    ...
                    for pid_str, player_data in players_data.items():
                        ...
                        if pid != self.player_id:
                            ...
                            # Include direction and moving state so clients can
                            # render proper animation and movement state.
                            dir_val = player_data.get("dir", None)
                            moving_val = player_data.get("moving", False)
                            try:
                                moving_val = bool(moving_val)
                            except Exception:
                                moving_val = False
            ...
            try:
                ...
                if now - last_update >= update_interval:
                    ...
                    if latest_update and self.player_id >= 0:
                        # HINT: This part might be helpful for direction change
                        # Maybe you can add other parameters? 
                        # Ensure direction is serializable (use name for Enums)
                        dir_val = latest_update.get("dir")
                        try:
                            dir_out = dir_val.name
                        except Exception:
                            dir_out = str(dir_val) if dir_val is not None else ""

                        message = {
                            "type": "player_update",
                            "x": latest_update["x"],
                            "y": latest_update["y"],
                            "map": latest_update["map"],
                            "dir": dir_out,
                            "moving": latest_update["moving"]
                        }
